...
def process_data(data_file, user_prompt="What is this?"):
    ...
    for index, row in tqdm(data_file_df.iterrows(), desc="Creating Data Set"):
        frames = []
        clip_dir = f"/Users/mario/Desktop/Desktop/UofA/4.Winter-2025/ece-910/Batman/VLM-testing/dyna/clip-{index}"
        ...
        if len(frames) == 0:
            print(f"⚠️ Skipping index {index} - No frames found.")
            continue

        # Fix column name issue (handling leading spaces)
        row_action = row.get(" Action", "No Action Found").strip()  

        toks = "<image>" * (len(frames[:10]))
    ...
    if not found_data:
        print("⚠️ No data was yielded from process_data(). Check earlier logs.")
